...
import ProfileSidebar from "@/app/(page)/profile/profile-sidebar";
    ...
    <main className="min-h-[calc(100vh-73px)] bg-[#050505] px-3 py-6 text-white sm:px-6 sm:py-10">
      <div className="mx-auto grid w-full max-w-7xl gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <section className={`${panelClassName} overflow-hidden`}>
          <div className="h-20 border-b border-white/8 bg-[#101010]" />
          <div className="px-4 pb-4 sm:px-6 sm:pb-6">
            <Skeleton className="-mt-10 h-20 w-20 rounded-[1rem] bg-white/10" />
            <Skeleton className="mt-5 h-8 w-40 bg-white/10" />
            <Skeleton className="mt-2 h-4 w-24 bg-white/8" />
            <div className="mt-6 grid grid-cols-2 gap-3">
              {[1, 2].map((item) => (
                <div
                  key={item}
                  className="rounded-[0.85rem] border border-white/8 bg-[#0f0f0f] p-4"
                >
                  <Skeleton className="h-3 w-14 bg-white/10" />
                  <Skeleton className="mt-4 h-7 w-12 bg-white/8" />
                </div>
              ))}
            </div>
          </div>
        ...
        <section className={`${panelClassName} p-6 sm:p-8`}>
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <Skeleton className="mt-4 h-9 w-56 bg-white/8" />
            </div>
          ...
          <div className="mt-6 space-y-3">
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="rounded-[0.95rem] border border-white/8 bg-[#0f0f0f] p-4"
              >
                <div className="flex flex-col gap-4 sm:flex-row">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <Skeleton className="mt-3 h-4 w-28 bg-white/8" />
                ...
                {reviewsLoading ? (
                  <div className="space-y-4">
                    {[1, 2, 3].map((item) => (
                      <div
                        key={item}
                        className="rounded-[0.95rem] border border-white/8 bg-[#0f0f0f] p-4 sm:p-5"
                      >
                        <div className="flex flex-col gap-4 sm:flex-row">
                          <Skeleton className="mx-auto aspect-[0.7] w-full max-w-[132px] rounded-[0.7rem] bg-white/8 sm:mx-0 sm:h-[148px] sm:w-[102px] sm:max-w-none sm:aspect-auto lg:h-[180px] lg:w-[124px]" />
                          <div className="min-w-0 flex-1">
